CLASS ltc_compute_rebate DEFINITION FINAL FOR TESTING
  ...
  PRIVATE SECTION.
    ...
    METHODS no_rebate_for_one_item_in_cart FOR TESTING RAISING cx_static_check.
    METHODS tot_rebate_for_2_items_in_cart FOR TESTING RAISING cx_static_check.
ENDCLASS.
...
CLASS ltc_compute_rebate IMPLEMENTATION.
  ...
  METHOD setup.
    ...
    DATA(rules) = VALUE zcl_wtc_rebate_engine=>tt_rules(
      ...
      ( NEW zcl_wtc_rule_item_id( i_item_id = 2  i_req_quantity = 4  i_rebate = 10 ) ) ).
  ...
  METHOD no_rebate_for_empty_cart.
    ...
    "Then
    cl_abap_unit_assert=>assert_initial( m_cut->get_total_rebate_amount( c_cart_items = cart_items ) ).
  ENDMETHOD.
  ...
  METHOD no_rebate_for_one_item_in_cart.
    "Given
    DATA(cart_items) = VALUE zase_shop_cart( ( id = 11 quantity = 1 )
                                            ( id = 1 quantity = 1 )
                                            ( id = 2 quantity = 3 ) ).
    DATA(cart_items_exp) = cart_items.
    cart_items_exp[ 2 ]-rebate_amount = 0.
  ...
  METHOD tot_rebate_for_2_items_in_cart.
    "Given
    DATA(cart_items) = VALUE zase_shop_cart( ( id = 11 quantity = 1 )
                                            ( id = 1 quantity = 2 )
                                            ( id = 2 quantity = 4 ) ).
    ...
    "Then
    cl_abap_unit_assert=>assert_equals( act = m_cut->get_total_rebate_amount( c_cart_items = cart_items )
                                        exp = 15 ).
  ENDMETHOD.
