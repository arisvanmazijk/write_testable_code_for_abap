...
CLASS ltd_cash_provider DEFINITION FOR TESTING.
  ...
  PUBLIC SECTION.
    INTERFACES zif_wtc_cash_provider PARTIALLY IMPLEMENTED.
ENDCLASS.
...
CLASS ltc_get_amount_in_coins DEFINITION FOR TESTING
  ...
  PRIVATE SECTION.
    DATA m_cut TYPE REF TO zcl_wtc_depend_lookup.
...
CLASS ltc_get_amount_in_coins IMPLEMENTATION.
  ...
  METHOD setup.
    "given
    zcl_wtc_factory_injector=>inject_cash_provider( new ltd_cash_provider( ) ).
    m_cut = NEW #( ).
  ENDMETHOD.
